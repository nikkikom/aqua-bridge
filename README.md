...
6. **Config.** Edit `/etc/aqua-bridge/config.yaml`: `mpc.channels` /
   ...
   on the aquaero, without needing to also command one of that device's
   aquabus outputs (`pwm5..8`): its aquabus flow slot is a second witness
   of the bus device's presence that needs no fan behind it either (§8
   item 130), so a sensor-only slave or a Quadro whose fans this daemon
   does not command is judged the same as one whose outputs are bound.
   While no device answers on the aquabus the slot then reads as missing
   instead of as the value the controller keeps serving for it, and the
   loss is reported in the device health),
   `onewire.sensors`
...
8. **1-Wire sensor commissioning** (DS18B20, once, before the daemon
   ...
   < 1 % and a cycle under `max_age_s / 2`. One reader per bus is the
   contract: two cycles overlapping on one bus master eat each other's
   readings — six times the cycle time and a third of the reads gone
   (PROJECT.md §8 item 39) — so `--check` tries to take the same
   cross-process lock the daemon holds while its reader threads run
   (`onewire.lock_path`) before it drives a single cycle, and **refuses to
   measure, saying why, if it cannot**: held by the daemon, or simply
   unknown either way. `--force` measures anyway, wrong numbers and all, for
   the rare case that calls for it. Reads that answer nothing at all are
   what a second reader looks like if one does get past that guard (e.g.
   `--force`, or a daemon whose lock could not be taken), and they are
   counted and named apart from an errno or an unusable value. A sensor
   that is on no bus prints `no read attempted`, never a 0 % failure rate.
   The tiers are `netlink` (one conversion for the whole
