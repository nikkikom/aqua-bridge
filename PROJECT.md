...
meanwhile runs no PBKDF2; refusals are logged once per window); the worker thread's raised nice value (Linux)
...
- **Every route** — `GET /`, every `/api/...` route and unknown paths —
  ...
  count. The page's 2 s poll is answered from the cache (on the event
