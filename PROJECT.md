...
`unknown_keys`, `stuck`, `stuck_latch`; DAS: `per_temp[name]` is `false`
while `name` is confirming (`sensor_confirm`) even when the gate's own raw
check passed this tick, so this outward copy never claims a value is
trusted when the estimator and the solver do not use it -- item 63; the
internal verdict `zones.py` reads is unaffected), `time` (`status`, `dt_obs`,
...
63. **Done** (2026-09-16): the outward copy of `diagnostics["gate"]
    ["per_temp"]` (`control/mpc.py`, DAS only) now reads `false` for a
    ...
    trusted while the estimator and the solver ignore it; the internal
    verdict `zones.py` reads for zone trust and fault closure is
    untouched (`tests/test_sensor_confirm.py`).
