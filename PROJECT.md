...
`tests/test_inputs_smart.py` (`POST /api/in/smart`). The suites skip
themselves when binding a localhost socket is denied (some
...
6. Config. For the DAS, copy the example by hand (the installer only
   ...
   host and credentials, `http.enabled` / `mqtt.enabled`. Every name must
...
12. `sudo systemctl enable --now aqua-bridge`; `journalctl -u aqua-bridge`
    ...
    `stale` or `prior`); `/api/health` and, in DAS mode, `/api/estimate`
    (every bay occupied or empty as physically true, no zone in fault) if
    HTTP is enabled; MQTT entities in HA; Digole later.
...
the PR jobs take about 7–9 min (`test (latest)`) and 5–9 min
(`test (pi-parity)`). A nightly failure prints a `@reproduce_failure`
blob in the log.
